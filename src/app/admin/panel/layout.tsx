...
import { Bot, Settings, LogOut, Menu, X, Database, ShoppingBag, Tag, Globe } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: "Inventario Bots", href: "/admin/panel/dashboard?tab=inventory", icon: Database },
    { name: "Catálogo de Mercado", href: "/admin/panel/dashboard?tab=market", icon: Globe },
    ...
    <div className="min-h-screen bg-[#0b0818] text-white flex flex-col md:flex-row">
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-[#0f0d1e] border-b border-white/5">
        <div className="flex flex-col">
          <h1 className="text-sm font-black uppercase tracking-wider">Jabbu Admin</h1>
          {adminUser && (
            <span className="text-[9px] text-[#84849b]">@{adminUser.username}</span>
          )}
        </div>
      ...
      `}>
        <div>
          <div className="mb-10 hidden md:block">
            <h1 className="text-lg font-black uppercase tracking-wider text-white">Jabbu Store</h1>
            <p className="text-[10px] font-bold text-[#84849b] uppercase tracking-widest mt-0.5">Panel de Control</p>
          </div>
          ...
          {adminUser && (
            <div className="bg-white/[0.02] border border-white/5 rounded-xl p-3 mb-3">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-9 h-9 rounded-xl bg-accent/10 border border-accent/20 flex items-center justify-center font-sans font-black text-xs text-accent">
                  {adminUser.username?.[0]?.toUpperCase() || 'A'}
                </div>
                <div>
                  <span className="text-sm font-black text-white block">@{adminUser.username}</span>
              ...
              <button 
                ...
                className="flex items-center justify-center gap-2 w-full py-2 rounded-lg text-xs font-bold text-red-400 bg-red-500/5 hover:bg-red-500/10 border border-red-500/10 transition-colors"
              >
