...
function StyledInput(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className="w-full px-4 py-3 bg-white/[0.03] border border-white/8 rounded-xl text-sm text-white placeholder-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/[0.05] transition-all"
    />
  );
}
...
function SaveButton({ saving, saved, label }: { saving: boolean; saved: boolean; label: string }) {
  return (
    <button
      type="submit"
      className="mt-2 px-6 py-3 bg-accent hover:bg-accent/90 rounded-xl text-xs font-black uppercase tracking-wider text-white transition-all flex items-center gap-2 shadow-[0_4px_20px_rgba(217,70,239,0.25)] hover:shadow-[0_4px_30px_rgba(217,70,239,0.4)] disabled:opacity-60 cursor-pointer"
    >
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`w-full flex items-center justify-between gap-4 px-4 py-3 bg-white/[0.03] border transition-all duration-200 rounded-xl text-sm text-white ${
          open ? "border-accent shadow-[0_0_15px_rgba(217,70,239,0.15)]" : "border-white/8 hover:border-white/20"
        }`}
      >
        <span className="font-bold">{options.find((o) => o.value === value)?.label}</span>
        <ChevronDown className={`h-4 w-4 text-white/40 transition-transform duration-200 ${open ? "rotate-180 text-accent" : ""}`} />
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div className="absolute top-full left-0 mt-1 w-full bg-[#110f1e] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-20">
            <div className="py-1">
              {options.map((option) => (
                <button
                  key={option.value}
...
export default function AdminSettingsPage() {
  ...
  const handlePricingSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSavingPricing(true);
    try {
      ...
      setSavedPricing(true); setTimeout(() => setSavedPricing(false), 2500);
    } finally { setSavingPricing(false); }
  };

  const handleUserSellSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSavingUserSell(true);
    try {
  ...
  const handleMinSellSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSavingMinSell(true);
    try {
      const res = await fetch(`${BACKEND_URL}/admin/marketplace/settings/minimum-sell-price`, {
        method: "PATCH", credentials: "include",
        headers: { "Content-Type": "application/json", "X-Tunnel-Skip-AntiPhishing-Page": "true" },
        body: JSON.stringify({ minimumUserSellPrice: Number(settings.minimumUserSellPrice) }),
      });
      if (!res.ok) throw new Error();
      setSavedMinSell(true); setTimeout(() => setSavedMinSell(false), 2500);
    } finally { setSavingMinSell(false); }
  };
  ...
  const handleWebhookSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      setSavedWebhook(true); setTimeout(() => setSavedWebhook(false), 2500);
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl space-y-8">
      ...
      {/* Page header */}
      <div>
        <h1 className="text-xl font-black uppercase tracking-wider text-white">Configuración Global</h1>
        <p className="text-xs text-[#84849b] mt-1">Ajustá las reglas del marketplace desde acá.</p>
      </div>

      {/* Tab bar */}
      <div className="flex flex-wrap gap-2 p-1.5 bg-white/[0.02] border border-white/[0.06] rounded-2xl">
        {TABS.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer flex-1 justify-center
                ${active
                  ? "bg-accent text-white shadow-[0_4px_20px_rgba(217,70,239,0.35)]"
                  : "text-[#84849b] hover:text-white hover:bg-white/[0.04]"
                }`}
            >
              <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${active ? "opacity-100" : "opacity-60"}`} />
              {tab.label}
            </button>
          );
        })}
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-accent/5 border border-accent/15 rounded-xl">
        {(() => { const t = TABS.find(t => t.id === activeTab)!; const Icon = t.icon; return (
          <>
            <Icon className="w-3.5 h-3.5 text-accent flex-shrink-0" />
            <span className="text-xs text-accent/80 font-semibold">{t.desc}</span>
          </>
        ); })()}
      </div>
          ...
          <form onSubmit={handlePricingSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <CustomDropdown
                ...
                onChange={(v) => setSettings({ ...settings, globalPriceModifierType: v })}
            ...
            <SaveButton saving={savingPricing} saved={savedPricing} label="Guardar Reglas de Precio" />
          </form>
        </div>
      )}
          ...
          <form onSubmit={handleUserSellSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <CustomDropdown
                ...
                onChange={(v) => setSettings({ ...settings, userSellModifierType: v })}
                options={MODIFIER_OPTIONS}
        ...
        <div className="bg-[#110f1e]/40 border border-white/[0.06] rounded-2xl p-6">
          ...
          <form onSubmit={handleMinSellSubmit} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-accent/60 text-sm font-bold">$</span>
                <input
                  type="number"
                  step="0.01"
                  value={settings.minimumUserSellPrice}
                  onChange={(e) => setSettings({ ...settings, minimumUserSellPrice: Number(e.target.value) })}
                  className="w-full pl-8 pr-4 py-3 bg-white/[0.03] border border-white/8 rounded-xl text-sm text-white placeholder-white/20 focus:outline-none focus:border-accent/50 focus:bg-white/[0.05] transition-all"
                />
              </div>
              <p className="text-[10px] text-[#84849b] font-mono">Los usuarios no podrán listar skins por debajo de este monto.</p>
            </div>
            <SaveButton saving={savingMinSell} saved={savedMinSell} label="Guardar Precio Mínimo" />
          </form>
        </div>
      )}
          ...
          <form onSubmit={handleWebhookSubmit} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-[#84849b] font-mono leading-relaxed">
                Tu servidor recibirá un <span className="text-white font-bold">POST</span> con los detalles del pedido, datos del cliente y método de pago en formato JSON.
              </p>
            </div>

            {/* Info card */}
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.05] space-y-1.5">
              <p className="text-[10px] font-black uppercase tracking-widest text-[#84849b]">Eventos disparados</p>
              <div className="flex flex-wrap gap-2 mt-2">
                {["order.created", "order.status_updated"].map(ev => (
                  <span key={ev} className="px-2 py-1 bg-accent/10 border border-accent/20 text-accent text-[10px] font-mono rounded-lg">
                    {ev}
                  </span>
                ))}
              </div>
            ...
            <SaveButton saving={savingWebhook} saved={savedWebhook} label="Guardar Webhook" />
          </form>
        </div>
      )}
