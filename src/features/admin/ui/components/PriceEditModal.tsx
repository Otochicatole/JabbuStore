...
import { X, ToggleRight, ToggleLeft, DollarSign, XCircle, Loader2, CheckCircle2 } from 'lucide-react';
...
export function PriceEditModal({ item, onClose, onSuccess }: PriceEditModalProps) {
  const [manualEnabled, setManualEnabled] = useState(item.isPriceManual ?? false);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/admin/marketplace/store/items/${item.assetId}/price`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'X-Tunnel-Skip-AntiPhishing-Page': 'true',
        },
        body: JSON.stringify({ price: parsedPrice, isPriceManual: manualEnabled }),
        credentials: 'include'
      });
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            {item.iconUrl ? (
              <img src={item.iconUrl} alt={item.name} className="w-10 h-8 object-contain" />
            ) : (
              <div className="w-10 h-8 bg-white/5 rounded-lg" />
            )}
            <div>
              <p className="text-xs font-black text-white line-clamp-1">{item.name}</p>
              <p className="text-[10px] text-[#84849b] font-mono mt-0.5">{item.exterior ?? item.type}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 space-y-6">
          ...
          {/* Input de precio */}
          <div className={`space-y-2 transition-opacity duration-200 ${manualEnabled ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
            ...
            <p className="text-[10px] text-[#84849b] font-mono">
              Precio actual del mercado: <span className="text-white font-bold">${item.price.toLocaleString()}</span>
            </p>
          </div>
