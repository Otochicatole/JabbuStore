...
export const InventoryCard = ({
  ...
  const isUnsellable =
    variant === "sell" &&
    (skin.weapon.toLowerCase().includes("name tag") ||
      skin.weapon.toLowerCase().includes("etiqueta de nombre") ||
      skin.weapon.toLowerCase().includes("storage unit") ||
      skin.weapon.toLowerCase().includes("unidad de almacenamiento") ||
      skin.name.toLowerCase().includes("name tag") ||
      skin.name.toLowerCase().includes("etiqueta de nombre") ||
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDisabled ? "cursor-not-allowed opacity-60" : variant === "sell" ? "cursor-pointer hover:-translate-y-1" : ""}
        ${isSelected && variant === "sell" ? "border-accent shadow-[0_0_20px_rgba(217,70,239,0.2)]" : isDisabled ? "border-white/5" : "border-white/5 hover:border-white/10"}
      `}
    >
      {/* Blur overlay for disabled items */}
      {isDisabled && (
      ...
      <div className="mb-2">
        <h2 className="text-[9.5px] font-black text-white leading-normal line-clamp-1 uppercase tracking-tight py-0.5">
          {skin.isStatTrak && (
