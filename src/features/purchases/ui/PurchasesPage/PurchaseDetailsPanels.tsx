...
export function PurchaseDetailsPanels({
  ...
  return (
    <>
      ...
      <div className="bg-[#110f1e]/40 p-5 border border-white/10 rounded-[3px]">
        <span className="text-[10px] font-black uppercase text-[#84849b] tracking-wider font-mono block mb-3">
        ...
        {visibleProof && visibleProofUrl ? (
          <button
            ...
            className="w-full sm:w-auto flex items-center gap-2 px-4 py-2.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 rounded-[3px] hover:bg-emerald-500/15 transition-colors cursor-pointer"
          >
          ...
          <p className="text-sm text-white/35 font-bold">
            {isBuy ? t("purchases.noBuyerProof") : t("purchases.noAdminProof")}
          </p>
        )}
      </div>

      <div className="bg-[#110f1e]/40 p-5 border border-white/10 rounded-[3px] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <span className="text-[10px] font-black uppercase text-[#84849b] tracking-wider font-mono block mb-1">
            {t("tickets.support")}
          </span>
          <p className="text-sm text-white/35 font-bold">{t("tickets.orderHelp")}</p>
        </div>
        <button
          type="button"
          onClick={onOpenSupport}
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 bg-accent/10 hover:bg-accent/15 border border-accent/20 text-accent rounded-[3px] text-xs font-black uppercase transition-all cursor-pointer shrink-0"
        >
...
function PaymentInfoPanel({ order, t }: { order: Order; t: Translate }) {
  ...
  return (
    <div className="bg-[#110f1e]/40 p-5 border border-white/10 rounded-[3px]">
      ...
      <div className="text-xs text-[#84849b]">
      ...
      {arsQuote && (
        <div className="mt-3 rounded-[3px] border border-accent/20 bg-accent/10 p-4 text-[11px]">
...
function PaymentFields({ fields }: { fields: [string, string][] }) {
  return (
    <div className="grid grid-cols-2 gap-3 mt-2 pt-2 border-t border-white/10 text-[11px]">
      {fields.map(([label, value]) => (
        <div key={label}>
          <span className="text-[10px] text-[#84849b] block">{label}</span>
...
function ManualTransferPanel({ order, t }: { order: Order; t: Translate }) {
  ...
  return (
    <div className="bg-emerald-500/10 p-5 border border-emerald-500/20 rounded-[3px]">
      <span className="text-[10px] font-black uppercase text-emerald-300 tracking-wider font-mono block mb-3">
        {t("paymentMethod.manual_transfer.name")}
      </span>
      <p className="text-xs text-emerald-100/70 font-bold uppercase tracking-wider mb-3">
...
function ManualField({
  ...
  return (
    <div className={className}>
      <span className="text-[#84849b] uppercase block text-[10px]">{label}</span>
      <span className="font-mono font-bold text-white break-all text-sm">{value}</span>
    </div>
  );
}
...
function BotAccountPanel({ order, t }: { order: Order; t: Translate }) {
  ...
  return (
    <div className="bg-[#110f1e]/40 p-5 border border-white/10 rounded-[3px]">
      <span className="text-[10px] font-black uppercase text-[#84849b] tracking-wider font-mono block mb-2">
        {isBuy
      ...
      <p className="text-xs text-white/50 mb-3 font-semibold">
        {isBuy
      ...
      <div className="grid grid-cols-2 gap-3 text-[11px]">
        <div>
          <span className="text-[10px] text-[#84849b] block uppercase">{t("purchases.botAccountName") || "Nombre del Bot"}</span>
          <span className="font-bold text-white block select-all text-sm">{order.bot.name}</span>
        </div>
        <div>
          <span className="text-[10px] text-[#84849b] block uppercase">{t("purchases.botAccountSteamId") || "Steam ID"}</span>
          <span className="font-mono font-bold text-accent block select-all text-sm">{order.bot.steamId}</span>
        </div>
      </div>
      {order.bot.tradeUrl && (
        <div className="mt-3 pt-2.5 border-t border-white/10">
          <a
            ...
            className="text-[11px] font-bold uppercase tracking-wider text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1 cursor-pointer"
          >
