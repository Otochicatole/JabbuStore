...
interface AdminDropdownProps<T> {
  value: T;
...
export function AdminDropdown<T extends string>({
  ...
  options,
  labelPrefix,
  widthClass = "min-w-[140px] sm:min-w-[180px]",
  ...
  return (
    <div className={`relative z-20 w-full sm:w-auto ${widthClass}`} ref={containerRef}>
      <div className="flex items-center justify-between sm:justify-start gap-2 w-full">
        {labelPrefix && (
          <span className="text-[9px] sm:text-[10px] font-bold text-[#84849b] uppercase tracking-widest shrink-0">
            {labelPrefix}:
          </span>
        )}
        <button
        ...
        {isOpen && (
          <>
            <motion.div
              ...
              className="absolute right-0 top-full mt-2 w-full min-w-[160px] bg-card border border-white/10 rounded-xl overflow-hidden shadow-2xl z-30 backdrop-blur-xl"
            >
