...
export function AdminOrderTicketsModal({
    ...
    const fetchTickets = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setError("Error al cargar los tickets de esta orden.");
        }
      } catch (err) {
        console.error(err);
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    if (subject.trim().length < 3 || subject.trim().length > 120) {
      setError("El asunto debe tener entre 3 y 120 caracteres.");
      return;
    }
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black uppercase tracking-wider text-white">
                Soporte / Chat de Orden
              </h3>
              <p className="text-[10px] text-[#84849b] font-mono mt-0.5">
                {orderType === "BUY" ? "Compra" : "Venta"} #{orderId.slice(0, 8)} - ${orderPrice.toFixed(2)} USD
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-4">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-10">
              ...
              <p className="text-xs text-[#84849b] font-bold uppercase tracking-wider">Cargando tickets...</p>
            </div>
          ) : (
            <>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-black uppercase text-[#84849b] tracking-wider font-mono">
                      Tickets existentes ({tickets.length})
                    </span>
                    <button
                      onClick={() => {
                        setShowCreate(true);
                      ...
                      >
                        <div className="min-w-0 flex-1 pr-3">
                          ...
                          <span className="text-[9px] text-[#84849b] font-mono">
                            Creado el {new Date(t.createdAt).toLocaleDateString("es-AR", {
                              day: "2-digit",
                              month: "short",
                              year: "numeric",
                              hour: "2-digit",
                        ...
                        <button
                          onClick={() => {
                            window.location.href = `/admin/panel/tickets?ticket=${t.id}`;
                          }}
                ...
                <form onSubmit={handleCreateTicket} className="space-y-3.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      type="text" 
                      required
                      placeholder="Ej. Problema con la cotización / transferencia"
                      value={subject}
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] uppercase font-black tracking-wider text-[#84849b]">
                      Primer Mensaje / Detalles
                    </label>
                    <textarea 
                      required
                      rows={4}
                      placeholder="Escribe el mensaje detallado para el usuario aquí..."
                      value={message}
